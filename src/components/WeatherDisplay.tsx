...
const WeatherDisplay: React.FC<WeatherDisplayProps> = ({ weatherData, backgroundGradient, isLight, location }) => {
  ...
  const hourlyForecastBackground = isLight ? 'bg-white bg-opacity-30' : 'bg-black bg-opacity-30';
  ...
  const isLocationSaved = locations.some(
    loc => loc.latitude === weatherData.lat && loc.longitude === weatherData.lon
  );
      ...
      <div className="max-w-7xl mx-auto p-4">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col items-center">
            ...
            {Math.round(weatherData.current.feels_like) !== Math.round(weatherData.current.temp)   && (
              <p className={`text-md mb-6 ${textColor} opacity-70`}>
                (but, <span className='italic'>it feels like <span className='font-bold'>{Math.round(weatherData.current.feels_like)}°C</span> </span>)
              </p>
            )} 
        ...
        <Tabs defaultValue="hourly" className="w-full mt-8">
          ...
          <TabsContent value="hourly">
            ...
            <div className={`mt-8 flex flex-wrap justify-between`}>
              ...
              <div className={`w-1/2 flex space-between justify-center flex-row`}>
                <div className={`rounded-lg p-4 ${hourlyForecastBackground} mr-4 w-1/2 max-h-48`}>
                  <SunriseSunsetChart
                ...
                {airQualityIndex && (
                  <div className={`rounded-lg p-4 w-1/2 ${hourlyForecastBackground} max-h-48`}>
                    <AirQualityDisplay
                      aqi={airQualityIndex}
                      textColor={textColor}
                    />
